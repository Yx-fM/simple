"""Ensemble prediction using all available models."""

import torch
import pandas as pd
import numpy as np
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent))

from model_cnn import CNN

def load_test_data():
    test_df = pd.read_csv('digit-recognizer/test.csv')
    test_images = test_df.values.astype('float32') / 255.0
    return test_images

def predict_ensemble():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Device: {device}")

    test_images = load_test_data()
    test_tensor = torch.tensor(test_images).view(-1, 1, 28, 28)
    print(f"Test samples: {len(test_images)}")

    models = []

    # Load advanced model (fold 0) - 99.68% val acc
    print("\nLoading advanced model (fold 0)...")
    model = CNN().to(device)
    model.load_state_dict(torch.load('model_adv_fold0.pth'))
    model.eval()
    models.append(('adv_fold0', model))

    # Load previous CNN fold models
    print("Loading previous CNN fold models...")
    for i in range(5):
        model = CNN().to(device)
        model.load_state_dict(torch.load(f'model_cnn_fold{i}.pth'))
        model.eval()
        models.append((f'cnn_fold{i}', model))

    # Load CNNv2 fold models
    print("Loading CNNv2 fold models...")
    for i in range(5):
        try:
            model = CNN().to(device)
            model.load_state_dict(torch.load(f'model_cnnv2_fold{i}.pth'))
            model.eval()
            models.append((f'cnnv2_fold{i}', model))
        except:
            print(f"  CNNv2 fold {i} not found, skipping")

    print(f"\nTotal models: {len(models)}")

    # Ensemble prediction
    all_predictions = []
    batch_size = 256

    with torch.no_grad():
        for start in range(0, len(test_tensor), batch_size):
            end = min(start + batch_size, len(test_tensor))
            batch = test_tensor[start:end].to(device)

            logits_list = []
            for name, model in models:
                logits = model(batch)
                logits_list.append(logits)

            avg_logits = torch.stack(logits_list).mean(dim=0)
            _, predicted = torch.max(avg_logits, 1)
            all_predictions.extend(predicted.cpu().numpy().tolist())

            if (start + batch_size) % 5000 == 0 or end == len(test_tensor):
                print(f"Processed {end}/{len(test_tensor)}")

    # Save submission
    submission = pd.DataFrame({
        'ImageId': range(1, len(all_predictions) + 1),
        'Label': all_predictions
    })
    submission.to_csv('submission.csv', index=False)
    print(f"\nSaved submission.csv with {len(all_predictions)} predictions")
    print(submission.head(10))


if __name__ == '__main__':
    predict_ensemble()